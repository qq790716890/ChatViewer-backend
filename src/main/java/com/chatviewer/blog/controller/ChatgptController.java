...
//    public String sendMsgString(@RequestBody Map<String, Object> map) {
...
//    }
    ...
    public String sendMsg(@RequestBody Map<String, Object> map) {
        // true 使用账户token数；false 使用账户自定义ApiKey
        Boolean useToken = (Boolean) map.get("useToken");
        Long conversationId = null;
        if (map.containsKey("conversationId") && StringUtils.isNotBlank((String) map.get("conversationId"))) {
            conversationId = Long.parseLong((String) map.get("conversationId"));
        }
        // 发送消息内容
        String prompt = (String) map.get("prompt");
        String ans = chatgptService.sendMsgString(useToken, conversationId, prompt);
